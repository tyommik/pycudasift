...
cdef class PySiftData:
    ...
    def from_data_frame(data_frame, features):
        ...
        score_off = <size_t>(&pts.score - <float *>pts)
        ambiguity_off = <size_t>(&pts.ambiguity - <float *>pts)
        tmp[:, xpos_off] = data_frame.xpos.to_numpy()
        tmp[:, ypos_off] = data_frame.ypos.to_numpy()()
        tmp[:, scale_off] = data_frame.scale.to_numpy()()
        tmp[:, sharpness_off] = data_frame.sharpness.to_numpy()()
        tmp[:, edgeness_off] = data_frame.edgeness.to_numpy()()
        tmp[:, orientation_off] = data_frame.orientation.to_numpy()()
        tmp[:, score_off] = data_frame.score.to_numpy()()
        tmp[:, ambiguity_off] = data_frame.ambiguity.to_numpy()()
        tmp[:, -128:] = features
        data.numPts = size
